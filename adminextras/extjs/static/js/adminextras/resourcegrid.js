 ...
 */
Ext.ux.ModelList = Ext.extend(Ext.grid.GridPanel, {
    constructor: function (config){
        ...
        this.verboseName = config.verboseName || Ext.ux.ModelList.verboseName;
        ...
        Ext.ux.ModelList.superclass.constructor.call(this, Ext.apply({
            store: store,
            columns: columns,
        ...
        var btn = new Ext.Button({
            ...
            handler: function (){
				var win = new Ext.Window({
					...
					closable: false,
					// Panel de creación
					items: new Ext.ux.CreateFormPanel({
 ...
 * @param {Object} config
 */
Ext.ux.PistonErrorReader = Ext.extend(Ext.data.JsonReader, {
	
	constructor: function (config){
		var fields = this.getFieldsFromStore(config.store);
		//console.info(fields);
		Ext.ux.PistonErrorReader.superclass.constructor.call(this, Ext.apply({
			fields: fields
		}, config));
	},
	/**
...
Ext.ux.CreateFormPanel = Ext.extend(Ext.form.FormPanel, {
	constructor: function (config){
		var formPanel = this; // Para clausuras
		Ext.ux.CreateFormPanel.superclass.constructor.call(this, Ext.apply({
			padding: 2,
			items: {
				itemId: 'placeholder',
				html: '<div></div>',
			...
			errorReader: new Ext.ux.PistonErrorReader({
				successProperty: 'success',
                root: 'errors',
				errors: 'errors',
				store: config.store
...
//		}, this);
	...
	loadForm: function() {
		...
		this.connection.request({
			method: 'GET',
			url: Ext.ux.CreateFormPanel.formUrl,
			params: {
	...
	reconstructFormFieldConfig: function (field_cfg, is_inline) {
        if (! is_inline) {
            var overrides = Ext.ux.CreateFormPanel.fieldOverrides;
        }
        else {
			console.log("Es INLINE Es INLINE");
            var overrides = Ext.ux.CreateFormPanel.inlineFieldOverrides;
        }
...
//        }
	...
	comboBoxStoreFactory: function (field){
		...
		var storeUrl = Ext.urlJoin(Ext.ux.CreateFormPanel.fkUrl, subURL),
...
Ext.apply(Ext.ux.CreateFormPanel, {
 ...
 * @param {Object} config
 */
Ext.ux.EditFormPanel = Ext.extend(Ext.ux.CreateFormPanel, {
	constructor: function (config){
		Ext.ux.EditFormPanel.superclass.constructor.call(this, Ext.apply({
